package pr1.a07.mitArrayList;												//Alexander Biesdorf
																//820460
import java.io.PrintWriter;
import java.util.*;

import pr1.a06.Person;
import pr1.a07.mitCollection.NameComparator;
import pr1.a07.mitCollection.YearComparator;
...
public class Personen {
	...
	public static void main(String[] args) {
		...
		ArrayList<Person> testPersonen = new ArrayList<Person>();
		...
		//Aufgabe d)
		Collection<Person> leseTest = new ArrayList<Person>(getPersonsFrom("./listen/testpersonen.txt"));
		out.println("Aufgabe d)");
		printPersons(leseTest,out);
		out.println();
		
		//Aufgabe e)
		Collection<Person> Komilitonen = new ArrayList<Person>(getPersonsFrom("./listen/kommilitonen.txt"));
		Collection<Person> Sportfreunde = new ArrayList<Person>(getPersonsFrom("./listen/sportfreunde.txt"));
	
		//alle Sportfreunde die auch Kommilitonen sind
		Collection<Person> SundK = new ArrayList<Person>();
		...
		Collection<Person> KaberNichtS = new ArrayList<Person>();
		...
		Collection<Person> TvereinigtK = new ArrayList<Person>();
		TvereinigtK.addAll(Komilitonen);
		TvereinigtK.addAll(testPersonen);
		printPersons(TvereinigtK, "./listen/TvereinigtK.txt");
		
		
		Collection<Person> PersonList = new ArrayList<Person>(createTestPersonliste()); 
		YearComparator c = new YearComparator();
		NameComparator d = new NameComparator();
		printPersons(PersonList, c, out);
		out.println();
		printPersons(PersonList, d, out);
	...
	public static Collection<Person> getPersonsFrom(String filename){
		...
		Collection<Person> result = new ArrayList<Person>(getPersonsFrom(in));
		in.close();
		return result;	
	}	
	
	public static Collection<Person> createTestPersonliste(){
